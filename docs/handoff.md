...
> 当前补充：2026-07-02（**v0.31.2，会话打开默认定位最新消息已修复**）：v0.31.1 修复图片查看器小图窗口最小尺寸；v0.31.2 实现决议 #192，会话列表 / 联系人 / 通知 / 托盘 / 震动等入口默认重载最新 50 条并滚到底部，历史搜索跳转仍定位目标消息。
...
**v0.1、v0.2、v0.3、v0.4/P1 主链路已完成，本地迭代至 v0.31.2；会话打开默认定位最新消息已修复**（对照 tech-design §12）。Windows / Debian / UOS 真实打包运行测试留给目标平台执行：
