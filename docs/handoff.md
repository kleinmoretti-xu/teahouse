...
**v0.1–v0.4/P1 主链路已完成，当前代码版本 v0.34.1**。**内网通兼容：仅设计落档，代码未写，决议 #199 暂缓、不排近期**（对照 tech-design §12）。Windows / Debian / UOS 真实打包运行测试留给目标平台执行：
...
| 讨论组 | LWW 元数据（rev+updatedTs）、同一信封逐成员扇出、need/info 补元数据；群管理按创建 IP / 可选管理密码+提示控制，有密码组在群成员面板内输入管理密码后可改名、添加成员或移出成员；群名变化写入幂等系统提示（改名人「你把群名…」、其他成员「备注/昵称把群名…」）；群聊图片/文件按在线成员逐个点对点传输 |
