...
| Field | Value |
|---|---|
| Current design | v1.65 for v0.53.1 |
...
| v0.54.0     | Group description and group announce (decision #290): `GroupMeta` gains `description` (≤ 200 chars) and `announce` (≤ 1024 chars); `GroupPatch` adds `set-description` / `set-announce` ops executable only by owner/admin or a password-holding member; codec enforces length limits on inbound `group.info`; SQLite v16 migration adds both columns; `GroupsService` writes idempotent system hints and broadcasts info on change; renderer `GroupPanel` exposes edit buttons with `GroupDescDialog` / `GroupAnnounceDialog` modal popups; 12 new tests added. Protocol v0.51, SQLite v16, version **0.53.1 → 0.54.0** |
...
- **2026-08-31, v1.65, decision #290:** group description and group announce. `GroupMeta` gains `description` (≤ 200 characters) and `announce` (≤ 1024 characters). `GroupPatch` gains `set-description` and `set-announce` operations, executable only by owner/admin or a password-holding member. The codec enforces length limits on inbound `group.info`. SQLite migration v16 adds both columns. `GroupsService` emits idempotent system hints and broadcasts info on change. Renderer shows edit buttons in `GroupPanel` with `GroupDescDialog` and `GroupAnnounceDialog` modal popups. Wire protocol remains v0.51; SQLite advances to v16. Repository version 0.53.1 → **0.54.0**.
