...
| #290 | Group description and group announce | Group owners, administrators, or password-holding members may set a group description (≤ 200 characters) and a group announcement (≤ 1024 characters) from the group panel. Empty text is omitted from display. Changes broadcast via `group.info` and produce idempotent system hints. The codec enforces length limits on inbound `group.info`. SQLite migration v16 adds both columns. Protocol remains v0.51; SQLite advances to v16. Ship as v0.54.0.                     |
