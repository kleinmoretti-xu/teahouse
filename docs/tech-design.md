...
| Win7 搜狗输入法候选窗固定在应用左上角 | Chromium 108 在 Win7 恒走 IMM32。#253 的 TSF 开关、#255 的主进程 `show()` 自愈、#256 的 textarea blur/focus 刷新均经源码复核或真机实测撤销。Chromium `IMM32Manager::MoveImeWindow()` 在 Win32 `GetFocus()` 句柄与目标窗口不一致时直接跳过候选窗和系统 caret 更新；决议 #257 在主窗口重新激活、主聊天 textarea 获焦时直接调用 `webContents.focus()`，把系统焦点同步到 Chromium 原生视图。同时 renderer 不再向 `body` 写 CSS `zoom`，主进程用 Electron `webContents.setZoomFactor()` 统一应用 100% / 110% / 125% 字体倍率；主窗口和设置窗口创建时先应用、`did-finish-load` 后按最新设置再次确认，设置广播同步两个现存业务窗口的倍率。由 Chromium WebContents 原生页面缩放链路统一换算 DOM caret、视口与屏幕坐标；待 Win7 搜狗真机回归确认 |
