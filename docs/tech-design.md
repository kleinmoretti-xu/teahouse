...
| Win7 搜狗输入法候选窗固定在应用左上角 | Chromium 108 在 Win7 恒走 IMM32。#253 的 TSF 开关、#255 的主进程 `show()` 自愈、#256 的 textarea blur/focus、#257 的 `webContents.focus()` 与 #258 的 composition 几何脉冲均经源码复核或 Win7 搜狗真机实测撤销。交叉测试确认同一 WebContents 的全局搜索、独立设置输入和同机 Chrome 正常，故障范围已收敛到聊天自定义多行编辑器。#259 的静态正常流基础 textarea 已在 Win7 搜狗真机验证候选窗定位正常。决议 #260 保持该真实输入层为 Chromium/IMM32 唯一 caret 几何来源：实体材质背景、不透明文字、普通内边距和无 composition 自定义处理；输入卡视觉通过移除诊断内框并与外层使用同一材质恢复。Win7 缺少彩色 emoji 字体，由绝对定位、`pointer-events:none`、不参与布局的 Twemoji 覆盖层绘制本地图形。#261 仅把真实 textarea 与展示层的字体栈首位恢复为 `PantryEmojiBlank`：emoji advance 固定 `1.3em`、普通文字回落微软雅黑，使输入图形、消息正文尺寸和原生 caret 步进一致；继续保留静态正常流、实体背景、不透明文字及无组合事件处理。#257 的 renderer CSS zoom 清理与主进程 `webContents.setZoomFactor()` 继续保留；Win7 真机需复测“连续多个 emoji 后继续中文组词”和多行滚动对齐 |
