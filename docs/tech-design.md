...
| Win7 搜狗输入法候选窗固定在应用左上角 | Chromium 108 在 Win7 恒走 IMM32。#253 的 TSF 开关、#255 的主进程 `show()` 自愈、#256 的 textarea blur/focus、#257 的 `webContents.focus()` 均经源码复核或 Win7 搜狗真机实测撤销。v0.44.8 新照片显示候选窗精确锚在茶话间客户区 `(0,0)`，说明 Chromium 已接收组合输入，但 selection bounds 仍为零值或陈旧值。决议 #258 将主聊天 textarea 从绝对覆盖改为占满容器的正常流布局；仅 Win7 在 `compositionstart/update` 事件内同步交替施加 0–1px 左内边距并立即读取布局矩形，避免连续事件的样式变化被 Vue 批处理合并，触发 Blink 重排编辑光标和 Chromium IMM32 caret bounds 更新，composition 结束复位，不修改文本、选区或焦点。#257 的 renderer CSS zoom 清理与主进程 `webContents.setZoomFactor()` 继续保留，让页面坐标只经过 Chromium 原生倍率换算；待 Win7 搜狗真机回归确认 |
