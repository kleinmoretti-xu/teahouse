...
| |                                                                                                                                                                                                              |
|---|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 状态 | v1.65；ARM64 Wayland 截图防崩溃（决议 #289）、群简介与群公告（决议 #290）；v0.54.0                                                                                                                           |
| 日期 | 2026-08-31                                                                                                                                                                                                   |
...
| v0.54.0       | 群简介与群公告（决议 #290）：`GroupMeta` 新增 `description`（≤ 200 字符）与 `announce`（≤ 1024 字符）；`GroupPatch` 新增 `set-description` / `set-announce`，仅群主/管理员可执行；codec 入站对 `group.info` 执行长度白名单校验；SQLite v16 迁移追加两列；`GroupsService` 变更写幂等系统提示并广播；renderer `GroupPanel` 新增入口按钮与弹窗；新增测试 12 例。协议 v0.51、SQLite v16，版本 **0.53.1 → 0.54.0** | shared/protocol、shared/ipc、net/codec、main/index、main/services/groups、main/store/group-repo、preload、renderer GroupPanel / GroupDescDialog / GroupAnnounceDialog / PantryIcon |
...
- 2026-08-31 v1.65 决议 #290：群简介与群公告。`GroupMeta` 新增 `description`（≤ 200 字符）与 `announce`（≤ 1024 字符）字段；`GroupPatch` 新增 `set-description` / `set-announce` 两个操作，仅群主/管理员可执行；codec 入站对 `group.info` 的 `description` / `announce` 执行长度白名单校验；SQLite v16 迁移追加两列；`GroupsService` 变更同步写系统提示并广播；renderer `GroupPanel` 新增入口按钮与弹窗，`GroupDescDialog` / `GroupAnnounceDialog` 为 Teleport 模态框。新增测试 12 例。协议 v0.51、SQLite v16 保持，版本 **0.53.1 → 0.54.0**。
