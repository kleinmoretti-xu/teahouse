...
| Win7 部分旧输入法 / 第三方输入法候选窗固定在应用左上角 | Chromium 108 在 Win7 恒走 IMM32（TSF 仅在 Win8+ 且特性开启时启用），#253 的 TSF 开关与 #255 的主进程 `show()` 自愈均已撤销：前者在 Win7 为空操作，后者会优先聚焦 Electron 提供的内容视图、无法保证走到 HWND `SetInitialFocus`，且会重复触发显示生命周期。决议 #256 改在主聊天原生 `textarea` 的安全焦点边界刷新输入状态，使 Chromium 从 `NONE` 回到可输入状态并重新发布 selection bounds，进而触发 `InputMethodWinImm32::OnCaretBoundsChanged`，再由 `IMM32Manager` 同步候选窗与系统 caret；保存并恢复选区/滚动，composition 期间跳过，仅 Win7 启用；待真机回归确认 |
...
- 2026-07-16 v1.50 决议 #256：撤销 #255 的 `show()` 自愈；Win7 主聊天 `textarea` 在输入焦点建立/窗口重新激活后刷新 Chromium 文本输入客户端，强制 IMM32 重取 caret bounds，保留选区与滚动并避开 composition。`AppInfo` 增加本机 `windows7` 标记，协议与 SQLite 不变。版本 0.44.6 → **0.44.7**。
