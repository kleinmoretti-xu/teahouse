...
| Win7 部分旧输入法 / 第三方输入法候选窗固定在屏幕左上角 | Chromium 108 在 Win7 恒走 IMM32（TSF 仅在 Win8+ 且特性开启时启用），#253 曾追加的 `disable-features=TSFImeSupport` 属空操作、已撤销（决议 #254）；源码级测试禁止再引入该无效开关。故障模型（决议 #255）：IMM32 定位链路（`ImmSetCandidateWindow` + 系统插入符 + MSAA 光标事件）要求 Win32 键盘焦点位于顶层窗口，焦点落入 Chrome Legacy Window（只转发普通键盘消息、不转发 WM_IME_*）即打字可上屏但候选窗粘性退回屏幕左上角。对策：`util/win7-ime-focus.ts` 焦点自愈——窗口获得焦点后延迟重走原生 show 流程（`HWNDMessageHandler::Show` 末尾 `SetInitialFocus` 钉回焦点），仅 Win7 安装；待真机回归确认 |
