...
| Win7 搜狗输入法候选窗固定在应用左上角 | Chromium 108 在 Win7 恒走 IMM32。#253 的 TSF 开关、#255 的主进程 `show()` 自愈、#256 的 textarea blur/focus、#257 的 `webContents.focus()` 与 #258 的 composition 几何脉冲均经源码复核或 Win7 搜狗真机实测撤销。交叉测试确认同一 WebContents 的全局搜索、独立设置输入和同机 Chrome 正常，故障范围已收敛到聊天自定义多行编辑器。决议 #259 在 Win7 分支改用静态正常流的基础 textarea，保留 `v-model`、键盘、粘贴和滚动事件，绕开 `PantryEmojiBlank`、Twemoji 镜像层、透明背景及 composition 自定义处理；其他平台继续使用完整编辑器。若真机恢复则逐项恢复 Win7 样式锁定触发项，若仍失败则对照 `contenteditable` 编辑路径。#257 的 renderer CSS zoom 清理与主进程 `webContents.setZoomFactor()` 继续保留；待 Win7 搜狗真机回归确认 |
