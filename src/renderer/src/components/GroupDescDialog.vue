...
const charCount = computed(() => desc.value.length)
const canSave = computed(() => !saving.value && desc.value.trim().length > 0)
const isOwnerOrAdmin = computed(() => props.group.canManage)

onMounted(() => {
  desc.value = props.group.description ?? ''
  void nextTick(() => {
    const el = document.querySelector('#group-desc-input') as HTMLTextAreaElement | null
    el?.focus()
  })
})
...
async function save(): Promise<void> {
  ...
  try {
    ...
    if (!updated) {
      feedback.value = isOwnerOrAdmin.value ? '保存群简介失败，请重试' : '无权限操作'
      return
    }
    emit('close')
  } catch {
    feedback.value = '保存群简介失败，请稍后重试'
  } finally {
    saving.value = false
  }
}
  ...
  <Teleport to="body">
    <div class="mask" @mousedown.self="emit('close')">
      <section class="dialog" role="dialog" aria-modal="true" tabindex="-1">
        ...
        <div class="body">
          <p class="hint">群简介将展示在群信息中，最多 200 字</p>
          <textarea
            id="group-desc-input"
            v-model="desc"
            maxlength="200"
