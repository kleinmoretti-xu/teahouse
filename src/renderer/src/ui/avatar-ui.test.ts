...
describe('自定义头像渲染入口', () => {
  it('裁剪弹窗挂到 body 并具备模态、Esc、拖动、滚轮和忙碌态', () => {
    ...
    expect(crop).toContain('@mousedown="startDrag"')
    expect(crop).toContain("window.addEventListener('mousemove', moveDrag)")
    expect(crop).toContain("window.addEventListener('mouseup', endDrag)")
    expect(crop).toContain("window.addEventListener('blur', endDrag)")
    expect(crop).toContain('imageWidth: props.source.width')
    expect(crop).toContain('imageHeight: props.source.height')
    expect(crop).not.toContain('naturalWidth')
    expect(crop).not.toContain('naturalHeight')
